...
class WorkflowActionRule(models.Model):
    ...
    customer_journal_id = fields.Many2one(
        'account.journal', default=lambda self: self.env['account.move'].with_context(
            default_move_type='out_invoice')._get_default_journal(), help='This journal will be used in the customer '
        'invoices generated by this document.')
    vendor_journal_id = fields.Many2one(
        'account.journal', default=lambda self: self.env['account.move'].with_context(
            default_move_type='in_invoice')._get_default_journal(), help='This journal will be used in the vendor '
        'bills generated by this document.')
    customer_account_id = fields.Many2one(
        'account.account', help='This account will be used in the customer invoices generated by this document. If '
        'this account is not set, will be used the default account.')
    vendor_account_id = fields.Many2one(
        'account.account', help='This account will be used in the vendor bills generated by this document. If this '
    ...
    analytic_account_id = fields.Many2one('account.analytic.account', help='Analytic account to be used in the '
    ...
    def _get_document_record(self, document, res_model, document_type, attachment):
        ...
        result = self.env[res_model].create(self._prepare_invoice_data(document_type, document))
    ...
    def _prepare_invoice_data(self, document_type, document):
        ...
        if cfdi is not False and cfdi.Emisor.get('Rfc') == cfdi.Receptor.get('Rfc'):
